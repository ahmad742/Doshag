...
export default class Add extends Component {
  ...
  add_Property = () => {
    ...
    Api.add_Property(JSON.stringify(bodyData))
      .then(
        function (response) {
  ...
  uploadImagesToServer = async () => {
    this.setState({ loading: true });
    console.log('UPLOADIMGIMAGE',JSON.stringify( this.state.list));
    ...
    fetch(constants.ApiBaseURL + 'local_image_storage', {
      ...
      .then((response) => {
        ...
        this.setState({ ImagesList: oldImages});
        this.add_Property();
      })
      .catch((err) => {
        this.setState({ loading: false });
