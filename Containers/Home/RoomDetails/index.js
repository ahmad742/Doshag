...
export default class index extends Component {
  ...
  componentDidMount() {
    console.log('this is prices', this.state.prices)
      ...
      if (value == date) {
        ...
        prices.map((value, index) => {
          if (moment(date).format('dddd') === value.day) {
            console.log("selected value", moment(date).format('dddd') + "-%%%%---" + value.day)
            let price = this.state.multiDatePrice + parseInt(value.price);
            console.log("selected value", this.state.multiDatePrice + "-%%^^^^%%---" + parseInt(value.price) + "-=======---" + parseInt(value.price))
            ...
            this.setState({
              ...
              console.log("selected value", this.state.booking_price)
  ...
  doPayment = () => {
    const { params } = this.props.navigation.state;
    const itemId = params ? params.roomdet.id : null;
    if (this.state.RoomsToBook < 1) {
      alert(`Please enter no of rooms to book`)
      return
    }
    if (this.state.RoomsToBook > this.state.availableRooms) {
      alert(`Cannot book room ${this.state.RoomsToBook} as there are only ${this.state.availableRooms} available`)
      return
    }
    ...
    this.setState({
      allConfigurations: {
        ...
        sessionParameters: {
          ...
          RoomsToBook: this.state.RoomsToBook,
  ...
  doBooking = (paymentResponse) => {
    // console.log("CHECK USER ID", Preference.get('userId'));
    // return
    ...
    const { params } = this.props.navigation.state;
    const RoomID = params ? params.roomdet.id : null;
    const RoomType = params ? params.roomdet.name : null;
    // return false;
    ...
    // } 
      ...
      let _RoomsToBook = 1
      const RoomsToBook = this.state.RoomsToBook
      if (RoomsToBook != "" && parseInt(RoomsToBook) > 0) {
        _RoomsToBook = parseInt(RoomsToBook)
      }
      ...
      let body = {
        ...
        room_id: RoomID,
        room_quantity: this.state.RoomsToBook,
        days: this.state.selectedDates.length,
        total_price: this.state.booking_price * _RoomsToBook,
        down_payment: Math.round(
          (20 / 100) * parseInt(this.state.booking_price * _RoomsToBook, 10),
        ),
        ...
        charge_id: "3213212365454",//paymentResponse.charge_id,
        payment_status: "success",// paymentResponse.sdk_result,
        start_time: ["8:00 am", "12:00 pm"],
        end_time: ["8:00 pm", "12:00 pm"],
        date: daySelected[0],

      }


      console.log('SendingData:----', body, paymentResponse);
  ...
  doBookingForGuest = (paymentResponse) => {
    // console.log("CHECK USER ID", Preference.get('userId'));
    // return
  ...
  get() {
    ...
    const roomId = params ? params.imageIndex : 0
    const property_id = params ? params.roomdet.property_id : -1
    let body = new FormData();


    console.log('*******************')
    console.log('roomId', itemId)
    console.log('property_id', property_id)

    console.log('*******************')


    body.append('room_id', itemId);
    //userId && body.append('user_id', userId);
    //console.log('BodyData', body);
    Api.roomDetailById(body)
      .then(
        function (response) {

          console.log('Property Details By ID:-- room', JSON.stringify(response));
          ...
          //   // }
          // });
          let listAmantiesTemp = mainResponse.room_amen;
          listAmantiesTemp = listAmantiesTemp.filter(
            (item) => item.quantity > 0,
          );

          let breakfastListData = mainResponse.prices
          console.log('this is breakfast list', breakfastListData)
          // const region = {
          ...
          // }
          this.setState({
            roomName: mainResponse.name,
            id: mainResponse.id,
            property_id: property_id,
            name_In_English: mainResponse.name,
            ...
            prices: mainResponse.prices,
            breakfastList: breakfastListData,
            //region: region,
            ...
            //noOfRooms: mainResponse.room_no,
            //country: mainResponse.country,
            //hotelRooms: mainResponse.rooms,
            availableRooms: parseInt(mainResponse.room_no),
            hotelRoomImages: mainResponse.room_images
          });
  ...
  onDayPressFunc = (day) => {
    ...
    let newSelectedDatesTemp = [...this.state.newSelectedDates]
    if (this.state.previousDay?.dateString == day.dateString && newSelectedDatesTemp.length > 1) {
      newSelectedDatesTemp = newSelectedDatesTemp.filter((_item) => _item.dateString != day.dateString)
    } else if (
      newSelectedDatesTemp.length > 1 &&
      newSelectedDatesTemp[0].day > day.day
    ) newSelectedDatesTemp[0] = day
    else if (newSelectedDatesTemp.length < 2) newSelectedDatesTemp.push(day)
    else newSelectedDatesTemp[1] = day

    if (newSelectedDatesTemp?.length == 0) {
      this.setState({
        rangeObject: {},
        newSelectedDates: newSelectedDatesTemp,
        previousDay: day
      })
      return
    }

    const startDay = newSelectedDatesTemp[0].day
    const endDay = day.day

    this.getDaysArray(new Date(moment(newSelectedDatesTemp[0].dateString).format('YYYY-MM-DD')), new Date(moment(day.dateString).format('YYYY-MM-DD')));
    let rangeObject = {}
    ...
    for (let index = startDay; index <= endDay; index++) {
      // console.log(endDay)
      let nextObject = { color: '#B20C11', textColor: "white" }
      if (index == startDay) {
        nextObject.startingDay = true
      } else if (index == endDay) {
        nextObject.endingDay = true
      }
      const dayString = `${day.dateString.substring(0, 8)}${index}`
      _selectedDates.push(`${dayString}`)
      rangeObject = {
        ...rangeObject,
        [dayString]: nextObject,
      }
    }
  ...
  getDaysArray = (start, end) => {
    ...
    for (var dt = new Date(start); dt < new Date(end); dt.setDate(dt.getDate() + 1)) {
      selectedDaysList.push(moment(dt).format('dddd'));
    }
    let booking_price = 0
    for (const _day of selectedDaysList) {
      const foundedItem = this.state.prices.find((_item) => _item.day == _day)
      if (foundedItem) {
        booking_price += parseFloat(foundedItem.price)
      }
    }
    this.setState({ booking_price: booking_price })
  };
  ...
  onDaySelect = (day) => {
    ...
    if (!dateContain) {
      ...
      this.setState({
        ...
        console.log("selectedDaya: ", JSON.stringify(this.state.selectedDates))
        this.calculatePriceByDay(day);
      });
    } else {
    ...
    return (
      <View
        ...
        <Image
          ...
          source={{ uri: "https://doshag.net/admin_clone/doshag_new/public/images/property_images/" + item.image }}
        />
        <TouchableOpacity
          onPress={() => {
            // //console.log('Hi?')
      ...
      <SafeAreaView
            ...
            <View style={styles.modalView}>
              <View style={{ flexDirection: "row", justifyContent: "space-between",alignItems:"center" }}>
                <Text
                  ...
                  {
                    lang == "en" ? "Enter Detail"
                      : lang == "ar" ? strings('register_screen.enter_detail') : null}
                  {/* {'Enter Detail'} */}
                </Text>
        ...
        <Header
          ...
          headerText={
            lang == 'en' ? this.state.name_In_English :
              lang == 'ar' ? this.state.name_In_Arabic : null
          }
          // {this.state.name_In_Arabic}
        ...
        <View
          ...
          <Carousel
            ...
            data={this.state.hotelRoomImages}
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={{ width: "100%" }}>
              <Text
                ...
                {strings('property_detail_screen.breakfast')}
              </Text>

              {
                <FlatList
                  data={this.state.breakfastList}
                  renderItem={({ item }) => (
                    item.breakfast > 0 ?
                      <View
                        style={{marginTop: 10 }}>
              ...
              <View>
                ...
                <View style={{ marginTop: 20, width: '100%' }}>
                  <FlatList
                    ...
                    keyExtractor={(item, index) => `listAmanties_${item?.id}_${index}`}
                ...
                <>
                  ...
                  {this.state.reviews.length == 0 ? (
                    ...
                    <View>
                      ...
                      <FlatList
                        keyExtractor={(item, index) => `reviewsView_${item?.id}_${index}`}
                        data={
                  ...
                  {this.state.showPopup == true ? (
                    <View style={{ flex: 1, marginHorizontal: 20 }}>
                      ...
                      <FlatList
                        keyExtractor={(item, index) => `prices_${item?.id}_${index}`}
                  ...
                  <Text
                    ...
                    {lang == 'en'
                      ? strings('property_detail_screen.no_of_rooms')
                      : lang == 'ar'
                        ? strings('property_detail_screen.no_of_rooms')
                  ...
                  <TextInput
                    value={this.state.RoomsToBook}
                    style={{ borderWidth: 1, borderColor: '#d3d3d3', marginTop: 10, height: 45, paddingHorizontal: 10 }}
                    keyboardType='number-pad'
                    placeholder='1'
                    placeholderTextColor='black'
                    onChangeText={(e) => { this.setState({ RoomsToBook: e }) }}
                  />
                  <View
                    style={{
                      flexDirection: 'row',
                    ...
                    <View>
                      <Calendar
                        ...
                        onDayPress={
                          ...
                          day => this.onDayPressFunc(day)

                        }
                        hideExtraDays={true}
                        // markedDates={this.state.markedDates}
                        markedDates={this.state.rangeObject}
                        markingType={'period'}
                        firstDay={1}
                  ...
                  <View
                    ...
                    <Text
                      ...
                      {Preference.get('currency') + " " + (this.state.booking_price * _RoomsToBook)}
                    </Text>
                  </View>
                  <View
                    style={{
                  ...
                  <View
                    ...
                    <Text
                      ...
                      {Preference.get('currency') + " " +
                        Math.round(
                          // console.log("kkkkk",this.state.booking_price)
                          (20 / 100) * parseInt(this.state.booking_price * _RoomsToBook),
                        )}
                    </Text>
                  </View>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      if (Preference.get('userLogin')) {
                        //  this.doBooking();
                        this.doPayment()
                        //this.doBooking(this.state.booking_price)
                      } else {
                        this.setState({ modalVisible: !modalVisible }, () => {
              ...
              this.state.Propertytype == 'Hotels' ?
                <View style={{ marginBottom: 50 }}>
                  ...
                  <FlatList
                    ...
                    keyExtractor={(item, index) => `hotelRooms_${item.id}_${index}`}
